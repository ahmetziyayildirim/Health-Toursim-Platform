...
import { Heart, MapPin, Calendar, DollarSign, User, MessageCircle, Star, Plane, Clock, Globe, Shield, Phone, Settings } from 'lucide-react';
...
const HealthTourismPlatform = () => {
  ...
  useEffect(() => {
    const loadAllPackages = async () => {
      try {
        ...
        if (response.success) {
          console.log('Setting packages:', response.data.length, 'packages');
  ...
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log('Auth form submit - Mode:', authMode);
      console.log('Auth form data:', authForm);
            ...
            {authMode === 'register' && (
              <>
                ...
                <input
                  ...
                  onChange={(e) => setAuthForm(prev => ({...prev, country: e.target.value}))}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  required
                />
              </>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loadingPackages ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {(featuredPackages.length > 0 ? featuredPackages : popularTrips).map((trip, index) => {
                // Handle both API data and fallback data
                const isApiData = trip._id !== undefined;
                const packageData = isApiData ? {
                  title: trip.title,
                  location: `${trip.location.city}, ${trip.location.country}`,
                  duration: trip.durationString || `${trip.duration.days} days`,
                  price: `€${trip.pricing.basePrice}`,
                  image: getPackageEmoji(trip.category),
                  rating: trip.rating.average
                } : trip;

                return (
                  <div key={isApiData ? trip._id : index} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow overflow-hidden cursor-pointer">
                    <div className="p-6">
                      <div className="text-4xl mb-4 text-center">{packageData.image}</div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-600 ml-1">{packageData.rating}</span>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  const renderPage = () => {
    switch(currentPage) {
      ...
      case 'admin': return user?.role === 'admin' ? <AdminPanel /> : renderHomePage();
